...
class MultiTaskModel(Sequential):
	...
	def train_on_batch(self,X,Y_image,Y_labels,optimizer,classification=False):
		...
		with tf.GradientTape(persistent=True) as tape:
			...
			if self.attention == 'multi':
				loss += self.regularizer_naive(result[-2],result[-1],indx=0)	#TODO Have this tunable
				loss += self.regularizer_naive(result[-4],result[-3],indx=0)
		...
		if self.two_stage==True and classification==True:
			...
			if self.attention=="self":
				for att in self.attention_gates_pred:
					g = tape.gradient(loss,att.trainable_variables)
					g_and_v = zip(g,att.trainable_variables)
					optimizer.apply_gradients(g_and_v)
			elif self.attention == "multi":
				g = tape.gradient(loss,self.attention_gates_pred.trainable_variables)
				g_and_v = zip(g,self.attention_gates_pred.trainable_variables)
				optimizer.apply_gradients(g_and_v)
